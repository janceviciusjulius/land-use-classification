...
def create_folder_for_results(working_dir):
    post_folder = os.path.join(os.path.dirname(working_dir),
...
def post_processing(raster_list, classification_dir, result_dir):
    ...
    conf_files = [os.path.join(classification_dir, conf_raster) for conf_raster in os.listdir(classification_dir)
                  if conf_raster not in raster_list and "ConfMap" in conf_raster
    ...
    try:
        ...
        for index, raster in enumerate(raster_list):
            for conf in conf_files:
                if (os.path.basename(raster).replace("Classified ", "")
                        == os.path.basename(conf).replace("ConfMap ", "")):
...
def clauses(conf_arr, raster_arr, month):
    mask = np.logical_or.reduce((
        np.logical_and(np.logical_and(conf_arr > 0.01, month in [4, 5, 6, 7, 8, 9, 10, 1]), raster_arr == 0),
        # April
...
if __name__ == "__main__":
    ...
    post_processing(raster_list=copied_rasters, classification_dir=data_directory, result_dir=postprocessing_folder)
