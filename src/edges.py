...
with rasterio.open(path_to_tiff) as dataset:
    # Read the image data for a single band
    band_index = 3  # Specify the band index (assuming it is a single-band image)
    image = dataset.read(band_index).astype(np.uint8)  # Read the band and convert to uint8

    # Apply Gaussian blur with a 5x5 filter
    blurred_image = cv2.GaussianBlur(image, (5, 5), 0)

    # Perform Canny edge detection
    edges = cv2.Canny(blurred_image, threshold1=250, threshold2=350)

    # Create a new GeoTIFF file for the edges
...
# edge_images = []
# for band in image:
#  edges = cv2.Canny(band, threshold1=100, threshold2=200) #again, keep in mind your threshold
#  edge_images.append(edges)
#
# # Combine the edge images into a single array
# combined_edges = np.stack(edge_images, axis=0)
#
# # Create a new GeoTIFF file for the combined edges
# output_path = 'output2.tiff'
# with rasterio.open(
#  output_path,
#  'w',
#  driver='GTiff',
